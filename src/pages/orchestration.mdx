...
And dynamic content loading? A CA can be made autonomous, as we described in another section, and prefetch content from multiple sources. Therefore, a consistent snapshot of the dynamic content can be loaded with one CA method call. Loading could be even faster with proactive server side rendering, described in the autonomous section.
