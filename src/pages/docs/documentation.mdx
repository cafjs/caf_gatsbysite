...
Therefore, dependencies between components are respected by controlling creation order.
...
caf.init(module);
```
pass `oops`, ('oops' < 0) is false and

transactional failures

not encouraging delaying assert

Note that we are **not** encouraging to delay input validation, but mistakes happen, and it is good to have a (partial) safety net.
