...
* **Irrecoverable** errors, mostly bugs, which *Caf.js* does not expect your code to handle gracefully. These are uncaught exceptions thrown by your code.
...
What happens if the input `n` is the string `oops`? The comparison with `0` is `false`, and we get a `NaN` after the addition. The assertion throws, creating an irrecoverable error.
...
#### Learn more
scratch, __ca_update,  __ca_resume
this.$.props
