...
* An **Application-level** error, i.e., the first element of the returned array, is used by the client library to reject the returned *Promise*. Therefore, `async` will throw, and the error can be managed inline.
...
What happens if we call multiple methods on the session without `await`? The session has a local queue, and serializes requests. Some recoverable errors are also managed transparently by retrying the last request. If you want multiple concurrent requests, create multiple sessions. But remember that the CA will serialize them anyway...
...
Encrypt messages end-to-end with Diffie-Hellman keys.

Synchronize local time with the Cloud.
