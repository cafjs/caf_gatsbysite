...
For external IP addresses, `cafjs run` internally uses the `xip.io` service, which redirects `*.192.168.1.15.xip.io` to `192.168.1.1`. Assuming that this address is your laptop external IP address, `192.168.1.15.xip.io` is your new wildcard domain.

What about `https`? In most cases you don't need it for development. For example, to enable WebXR APIs that need `localhost` or `https`, we use instead the Chrome developer tools that provide local port forwarding, and then connect the Oculus headset, or Android phone, to the laptop USB port. And now we can use a wildcard domain based on localhost, as shown in <Link to="three-way-isomorphic"> Three-way Isomorphic</Link>.

But if you really need `https`, you need a wildcard certificate, or a service like [ngrok](https://ngrok.com), and configure the `APP_SUFFIX` property with your new wildcard domain.

Let's go back to the simpler `http` case.
