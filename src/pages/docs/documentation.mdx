...
caf.init(module);
```
In this example, calling `install` by the `admin` owner creates a new *SharedMap* with a field `base`, and a method `computeLabel`. This method takes an argument `prefix`, looks up the key `base` in the `SharedMap`, obtains a random number, and adds them all up.
