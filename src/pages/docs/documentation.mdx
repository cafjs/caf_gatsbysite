...
and the simulated device in another host:

```shell
cafjs device -d --ipAddress 192.168.1.15 --port 8080 --password bar foo-device1
```

To use a real Raspberry Pi we adapt the startup script in [setup.sh](https://raw.githubusercontent.com/cafjs/caf_rpi/master/setup.sh), to first obtain a token from the local service (assume device id is `foo-device1`, password `bar`, and using the root account in the RPi)
