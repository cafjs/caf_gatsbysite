...
In fact, we anticipate that many developers will make their apps "free", by paying infrastucture costs themselves, and use other mechanisms to monetize their apps.

And you can decide what incremental resources are added for a newly subscribed customer. We give you feedback on whether you are wasting resouces, or starving your app. But whether to change the plan is up to you, and you can change it at anytime.

#### Domain-Specific PaaS

Every second, a busy cloud assistant uses less than one millisecond of compute, and checkpoints less than 500 bytes of state. The memory footprint increases by 200 KB, and we allocate 1.75 GB of extra egress traffic per year. This is our *Gold Plan*.

Assume a DIY hosting of this cloud assistant. How much does it cost per year in the Google Cloud?

A serverless platform like *Google Cloud Run* would cost $88/year (CPU: 31536000 invocations\*0.1\*$0.00002400, Request: 31.536\*$0.4), and a fast database for checkpointing, such as Cloud Firestore costs $56/year (315.36 writes\*$0.18). A total of **$144 per year**, and that ignores networking costs.
...
We built a **Domain-Specific PaaS** on top of Kubernetes for running **Caf.js** apps, and nothing else. We make strong assumptions on how these apps behave, and that simplifies the problem. By designing the framework and the runtime together, we can optimize beyond what is possible today in a general-purpose platform.
...
When you deploy your app using **Turtles**, it always starts in **Incubator** mode. In that mode resources are minimal, but they will grow when other users subscribe to your app.

The **Incubator** mode enables a very low maintenance cost for your new app. Currently less than **$6 per year** (one unit a week).

This means that with the free units, you could keep your app running for several weeks. If during that time a few people subscribe, you could achieve orbital velocity, paying baseline costs with subscriptions, and then keep growing...
