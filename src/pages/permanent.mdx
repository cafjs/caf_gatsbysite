...
**Caf.js** makes devices and web app instances (progressive or not) always accessible from anywhere at any time. An abstraction that we call reverse service worker keeps them safe. A three-way isomorphic framework enables quick prototyping of new sharing experiences.

<hr/>

Let's look at a few use cases first.
...
With **Caf.js** you can do that with a few hundred lines of JavaScript. There are a few examples already in GitHub, like a cheap ECG/Body temperature/SPO2 monitor [HealthyPi](https://github.com/cafjs/caf_healthypi.git), or a programmable beacon with many sensors [Puck.js](https://github.com/cafjs/caf_hellopuckjs.git), and it is easy to build your own with our app template generator tool, i.e., `cafjs generate yourapp iotbrowser`.

#### Integration of real devices in VR/AR

When a virtual world represents your real home, and action in VR could switch off the lights of your real home, thousands of miles away.

This could be implemented by creating an avatar of the lamp, which forwards actions to the physical lamp across the Internet.

However, this is not an easy problem to solve. Any delays or connectivity issues will ruin the virtual experience. And directly exposing your lamp to the Internet is risky.

Instead, **Caf.js** can take care of the backend for your avatar, ensuring safety and predictable latency, and then you can focus on making the avatar more realistic.

What about AR? The use case is very different, but our core technology is the same. You are likely to be in the same room, and when you look fixedly at the physical lamp, a user interface will pop up allowing you to change the color hue.
...
Where **Caf.js** really shines is when a team, with some members remote in VR, and others local in AR, collaborate by interacting with shared physical devices. With **Caf.js** all these user interfaces, and the state of the connected physical devices, are kept in sync.
...
An endpoint, for example, a device or app instance, connects to an RSW using a web socket. The RSW receives API requests from the Internet, performs security checks, updates its state, and then notifies the connected endpoint, or other client using web sockets. These notifications can then trigger actions in the physical world, or keep other user interfaces in sync.
